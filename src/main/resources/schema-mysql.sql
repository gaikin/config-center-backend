...
SET @sql := (
  SELECT CASE
    WHEN EXISTS (
      ...
      WHERE TABLE_SCHEMA = DATABASE()
        AND TABLE_NAME = 'page_menu'
        AND INDEX_NAME = 'uk_page_menu_menu_code'
    ) THEN 'DO 0'
    ELSE 'ALTER TABLE page_menu ADD UNIQUE KEY uk_page_menu_menu_code (menu_code)'
  END
);
PREPARE stmt FROM @sql;
EXECUTE stmt;
...
SET @sql := (
  SELECT CASE
    ...
    ELSE 'ALTER TABLE page_resource ADD COLUMN menu_code VARCHAR(64) NOT NULL AFTER id'
  END
);
PREPARE stmt FROM @sql;
EXECUTE stmt;
  ...
  SELECT CASE
    WHEN EXISTS (
      ...
      WHERE TABLE_SCHEMA = DATABASE()
        AND TABLE_NAME = 'page_menu'
        AND COLUMN_NAME = 'menu_code'
    ) THEN 'UPDATE page_resource pr JOIN page_menu pm ON pr.menu_id = pm.id SET pr.menu_code = pm.menu_code WHERE pr.menu_code IS NULL OR CHAR_LENGTH(TRIM(pr.menu_code)) = 0'
    ELSE 'DO 0'
  END
);
PREPARE stmt FROM @sql;
...
ALTER TABLE page_resource MODIFY COLUMN menu_code VARCHAR(64) NOT NULL;
...
SET @sql := (
  SELECT CASE
    ...
    ) THEN 'UPDATE page_resource SET menu_code = business_process_code WHERE (menu_code IS NULL OR menu_code = '''') AND business_process_code IS NOT NULL'
    ELSE 'DO 0'
  END
);
PREPARE stmt FROM @sql;
  ...
  SELECT CASE
    WHEN EXISTS (
      ...
      WHERE TABLE_SCHEMA = DATABASE()
        AND TABLE_NAME = 'page_resource'
        AND COLUMN_NAME = 'business_process_code'
    ) THEN 'ALTER TABLE page_resource DROP COLUMN business_process_code'
    ELSE 'DO 0'
  END
);
PREPARE stmt FROM @sql;
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='页面元素定义';
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='业务字段定义';
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='页面字段绑定';
...
SET @sql := (
  SELECT CASE
    WHEN EXISTS (
      ...
      WHERE TABLE_SCHEMA = DATABASE()
        AND TABLE_NAME = 'business_process'
    ) THEN 'INSERT IGNORE INTO page_menu (region_id, menu_code, menu_name, url_pattern, status, create_time, created_by, update_time, updated_by)
           SELECT ''business'', bp.process_code, bp.process_name, CONCAT(''/business/'', bp.process_code), ''ACTIVE'', CURRENT_TIMESTAMP, ''system.seed'', CURRENT_TIMESTAMP, ''system.seed''
           FROM business_process bp
           LEFT JOIN page_menu pm ON pm.menu_code = bp.process_code
           WHERE pm.id IS NULL'
    ELSE 'DO 0'
  END
);
  ...
  SELECT CASE
    WHEN EXISTS (
      ...
      WHERE TABLE_SCHEMA = DATABASE()
        AND TABLE_NAME = 'page_resource'
        AND COLUMN_NAME = 'business_process_code'
    ) THEN 'UPDATE page_resource
            SET menu_code = business_process_code
            WHERE business_process_code IS NOT NULL
              AND CHAR_LENGTH(TRIM(business_process_code)) > 0'
    ELSE 'DO 0'
  END
);
PREPARE stmt FROM @sql;
...
SET @sql := (
  SELECT CASE
    WHEN EXISTS (
      ...
      WHERE TABLE_SCHEMA = DATABASE()
        AND TABLE_NAME = 'business_process'
    ) THEN 'DROP TABLE business_process'
    ELSE 'DO 0'
  END
);
PREPARE stmt FROM @sql;
